...
<button id="report_lost" on:click={() => hash = "#camera"}>
...
<main>
    ...
    {:then value} 
        {#if disconected}
            <Throwerror inner="카메라가 연결되지 않았습니다."/>
        {/if}
        <div class="user-profile">
            <img class="user-profile-img" width="64" src={value.data[0].profile} alt="userprofile">
        </div>
    {/await}
        <div class="search">
            <input class="search-bar" bind:value={search} type="text" placeholder="아직 구현되지 않음.">
            <button class="search-button" on:click={() => search_onc()}>
