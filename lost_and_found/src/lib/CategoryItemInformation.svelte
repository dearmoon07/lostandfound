<script lang="ts">
    ...
    export let cateimg:string;
    export let catename:string;
    ...
    const lost_info = async() => {
        const res = await fetch(`/public/cate-${jsonname}.json`);
...
<main>
    ...
    {:then value} 
    <div class="header">
            <Backbtn bind:hash={hash} from="#category"></Backbtn>
            <h1>{catename}&nbsp;</h1>	
            <img src={cateimg} alt="sample">
    </div>
    <hr>
